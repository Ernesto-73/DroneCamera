...
void CCameraDlg::Draw(CDC * pDC)
{
	...
	if(m_arrOptions[AIM])
	{
		...
		CRect rec(m_point.x - sz, m_point.y - sz, m_point.x + sz, m_point.y + sz);
		pDC->Ellipse(&rec);
		pDC->MoveTo(m_point.x, m_point.y - static_cast<int>(1.5 * sz));
		pDC->LineTo(m_point.x, m_point.y + static_cast<int>(1.5 * sz));
		pDC->MoveTo(m_point.x - static_cast<int>(1.5 * sz), m_point.y);
		pDC->LineTo(m_point.x + static_cast<int>(1.5 * sz), m_point.y);
	}
...
cv::Mat CCameraDlg::HanleIamge()
{
	...
	cv::Scalar color(192,192,192);
	cv::Scalar color1(0,255,0);


	cv::line(img,p1,p3,color,2,8,0);
	cv::line(img,p2,p4,color,2,8,0);
	cv::circle(img,p0,60,color1,2,8,0);
